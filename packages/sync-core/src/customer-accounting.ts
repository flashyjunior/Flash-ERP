...
export function normalizeLoyaltyPolicy(policy?: Partial<LoyaltyPolicy> | null): LoyaltyPolicy {
  const normalizedPointsPerCurrencyUnit = Number(
    Number(policy?.loyaltyPointsPerCurrencyUnit ?? 1).toFixed(4)
  );
  const normalizedPointsStep = Math.max(
    1,
    Math.trunc(Number(policy?.loyaltyRedemptionPointsStep ?? 100))
  );
  const normalizedRedemptionValueAmount = roundMoney(
    Number(policy?.loyaltyRedemptionValueAmount ?? 1)
  );
  const normalizedMinimumRedeemPoints = Math.max(
    normalizedPointsStep,
    Math.trunc(Number(policy?.loyaltyMinimumRedeemPoints ?? 100))
  );
  const normalizedMaximumRedeemPercentOfSale = Number(
    Math.min(100, Math.max(0, Number(policy?.loyaltyMaximumRedeemPercentOfSale ?? 100))).toFixed(2)
  );

  return {
    loyaltyProgramEnabled: policy?.loyaltyProgramEnabled !== false,
    loyaltyPointsPerCurrencyUnit:
      Number.isFinite(normalizedPointsPerCurrencyUnit) && normalizedPointsPerCurrencyUnit > 0
    ...
    loyaltyRedemptionValueAmount:
      Number.isFinite(normalizedRedemptionValueAmount) && normalizedRedemptionValueAmount > 0
        ? normalizedRedemptionValueAmount
        : 1,
    loyaltyMinimumRedeemPoints: normalizedMinimumRedeemPoints,
    loyaltyMaximumRedeemPercentOfSale:
      Number.isFinite(normalizedMaximumRedeemPercentOfSale)
...
export function calculateLoyaltyRedemption(input: {
  customer:
    | Pick<CustomerAccountPostingCustomer, "fullName" | "loyaltyEnrolled" | "loyaltyPointsBalance">
    | null;
  totalAmount: number;
  requestedPoints: number;
  ...
  const valueStepsByBalance = Math.floor(
    Math.max(0, customer.loyaltyPointsBalance) / policy.loyaltyRedemptionPointsStep
  );
  const maxRedeemableAmountByPercent = roundMoney(
    totalAmount * (policy.loyaltyMaximumRedeemPercentOfSale / 100)
  );
  const valueStepsBySaleAmount = Math.floor(
    maxRedeemableAmountByPercent / policy.loyaltyRedemptionValueAmount
  );
  ...
  const maxRedeemableAmount = roundMoney(
    maxRedeemableSteps * policy.loyaltyRedemptionValueAmount
  );

  if (
    maxRedeemablePoints < policy.loyaltyMinimumRedeemPoints ||
    maxRedeemableAmount <= 0
  ) {
    return {
      policy,
      ...
      canRedeem: false,
      reason:
        maxRedeemableAmount <= 0 ? "NO_VALUE_AVAILABLE" : "BELOW_MINIMUM",
      message:
        maxRedeemableAmount <= 0
  ...
  const appliedPoints =
    Math.floor(requestedPointsClampedToMax / policy.loyaltyRedemptionPointsStep) *
    policy.loyaltyRedemptionPointsStep;
  const appliedAmount = roundMoney(
    (appliedPoints / policy.loyaltyRedemptionPointsStep) * policy.loyaltyRedemptionValueAmount
  );
  ...
  return {
    ...
    requestedPoints,
    appliedPoints:
      appliedPoints >= policy.loyaltyMinimumRedeemPoints ? appliedPoints : 0,
    appliedAmount:
      appliedPoints >= policy.loyaltyMinimumRedeemPoints ? appliedAmount : 0,
    maxRedeemablePoints,
    maxRedeemableAmount,
    canRedeem: true,
    reason: null,
...
export function deriveCustomerAccountPostingEffect(input: {
  ...
  const storeCreditTenderAmount = roundMoney(
    input.payments.reduce((sum, payment) => {
  ...
  const requestedRedeemedPoints = Math.max(
    0,
    Math.trunc(Number(input.loyaltyPointsRedeemed ?? 0))
  );
  ...
  if (!customer) {
    if (usesStoreCreditTender) {
      throw new Error(
        "Flash ERP needs an attached customer before Store Credit can be used on this basket."
      );
    }

    if (requestedRedeemedPoints > 0 || requestedRedemptionAmount > 0) {
      throw new Error(
        "Flash ERP needs an attached loyalty customer before points can be redeemed on this basket."
      );
    }
  ...
  if (customer.status.trim().toUpperCase() !== "ACTIVE") {
    throw new Error(
      `Flash ERP cannot post customer account activity because ${customer.fullName} is ${customer.status.toLowerCase()}.`
    );
  }
  ...
  if (usesStoreCreditTender) {
    if (normalizedTotalAmount > 0) {
      if (!customer.allowCreditSales) {
        throw new Error(
          `Flash ERP cannot use Store Credit for ${customer.fullName} because that customer is not credit-enabled.`
        );
      }
      ...
      if (roundMoney(customer.receivableBalanceAmount) < storeCreditTenderAmount) {
        throw new Error(
          `Flash ERP cannot refund ${customer.fullName} to Store Credit beyond the customer's outstanding receivable balance.`
        );
      }
  ...
  if (requestedRedeemedPoints > 0 || requestedRedemptionAmount > 0) {
    ...
    if (requestedRedeemedPoints !== loyaltyRedemption.appliedPoints) {
      throw new Error(
        `Flash ERP can only redeem ${loyaltyRedemption.appliedPoints} loyalty point(s) on this basket right now.`
      );
    }

    if (requestedRedemptionAmount !== loyaltyRedemption.appliedAmount) {
      throw new Error(
        "Flash ERP rejected the loyalty redemption because the amount did not match enterprise policy."
      );
    }
  }
  ...
  if (loyaltyPolicy.loyaltyProgramEnabled && customer.loyaltyEnrolled) {
    const basePoints = roundWhole(
      Math.abs(normalizedTotalAmount) * loyaltyPolicy.loyaltyPointsPerCurrencyUnit
    );
  ...
  return {
    ...
    nextReceivableBalanceAmount: roundMoney(
      customer.receivableBalanceAmount + receivableDeltaAmount
    ),
