...
export function trialSupportLoginId(workspaceSlug: string): string {
  return `${TRIAL_SUPPORT_LOGIN_PREFIX}${workspaceSlug.trim().toLowerCase()}`;
}

export function isTrialSupportLoginId(loginId: string | null | undefined): boolean {
  if (!loginId) return false;
  return loginId.trim().toLowerCase().startsWith(TRIAL_SUPPORT_LOGIN_PREFIX);
}

export function isTrialSupportRoleCode(roleCode: string | null | undefined): boolean {
