export type SecurityPermissionDomain =
  | "Master"
  | "Settings"
  | "Security"
  | "Inventory"
...
const catalog: SecurityPermissionDefinition[] = [
  {
...
export const securityPermissionCatalog = [...catalog].sort((left, right) =>
...
export function getSecurityPermissionDefinition(code: string) {
  return definitionsByCode.get(code) ?? null;
}

export function expandGrantedPermissionCodes(permissionCodes: string[]) {
...
export function groupSecurityPermissions(permissionCodes?: string[]) {
  const source =
...
export function deriveRetailUserCapabilities(permissionCodes: string[], accountStatus: string) {
  const expandedPermissions = new Set(expandGrantedPermissionCodes(permissionCodes));
  const isActive = accountStatus === "ACTIVE";
  const has = (...requiredCodes: string[]) =>
    requiredCodes.every((requiredCode) => expandedPermissions.has(requiredCode));
  const hasAny = (...candidateCodes: string[]) =>
    candidateCodes.some((candidateCode) => expandedPermissions.has(candidateCode));
  ...
  return {
    normalizedPermissionCodes: [...expandedPermissions].sort((left, right) =>
      left.localeCompare(right)
    ),
    cashierEligible: canOpenShift && canProcessSale,
    supervisorEligible:
      canApproveNoReceiptReturn || canApproveDiscountOverride || canApprovePriceOverride,
    canOpenShift,
    canCloseShift,
    canProcessSale,
    canProcessReturn,
