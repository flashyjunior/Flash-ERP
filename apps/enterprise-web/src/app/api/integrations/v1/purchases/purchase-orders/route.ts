...
import { getIntegrationPurchaseOrders } from "@/server/integrations/integration-queries";
...
export async function GET(request: Request) {
  try {
    ...
    const payload = await getIntegrationPurchaseOrders({
      searchParams: new URL(request.url).searchParams
    });

    return NextResponse.json(payload);
  } catch (error) {
    return createEnterpriseAuthErrorResponse(error, "Flash ERP could not export purchase orders.");
  }
}
