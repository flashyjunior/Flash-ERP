...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || file.size <= 0) throw new Error("Choose a receipt image before uploading.");
    if (file.size > maxUploadBytes) throw new Error("Receipt attachments must not exceed 10 MB.");
    ...
    const fileName = `${Date.now()}-employee-expense-${randomUUID()}${extension}`;
    ...
    return NextResponse.json({ message: error instanceof Error ? error.message : "Flash ERP could not upload the receipt." }, { status: error instanceof EnterpriseAuthError ? error.status : 400 });
  }
}
