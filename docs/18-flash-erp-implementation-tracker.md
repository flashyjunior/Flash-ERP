...
- HQ Reports now includes a Management category with exportable Executive Management Summary, Shop P&L Comparison, Cash Exposure, Customer/Supplier Exposure, Payroll Cost, and Management Exceptions reports derived from current sales, COGS, expense, cashbook, AR/AP, payroll, inventory, and sync-control data.
...
- Added a Management report category to the central HQ Reports workspace.
- Added an Executive Management Summary covering trading, costs, branch profitability, cash, working capital, payroll, inventory, and controls.
- Added Shop P&L Comparison rows with revenue, COGS, gross margin, tracked expenses, operating profit, receipt count, and status.
- Added Cash Exposure rows for cashbook balances, unreconciled entries, unreconciled value, and latest cashbook activity.
- Added Customer/Supplier Exposure rows combining open customer receivables and supplier payable exposure.
- Added Payroll Cost rows for payroll run gross pay, statutory liability, net pay, employer cost, employee count, status, and posted state.
- Added Management Exceptions rows consolidating store sync, closeout, cashbook, payroll, and slow-moving-stock attention items.
