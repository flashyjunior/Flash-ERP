...
- Current delivery position: central network allocation, internal-transfer routing, customer and staff visibility, configurable storefront slides, and Store Desktop order handoff are merged. VPS release `c183953-r1` was confirmed running with database readiness on 2026-08-24. Production-like UAT then exposed a same-shop ecommerce receipt projection conflict, premature Store Desktop handoff before staff acceptance, exact-only Item Dynamic filters, unreconciled ecommerce payment totals, and a terminal ecommerce reservation that remained active after fulfilment. The latest correction is committed and pushed at `231fc5a` and packaged as fresh VPS release `231fc5a-r1`; it reconciles terminal reservations and exposes per-location on-hand, active-reserved, safety-stock, ecommerce-eligibility, and web-sellable quantities in HQ, Online Store, and Store Desktop item views. External verification on 2026-08-25 confirmed the new database-readiness contract, both corrective migrations, sustained endpoint health, and the corrected ASUS storefront quantity of 3. VPS-local scheduled-task confirmation, the matching Store Desktop update, authenticated browser UAT, dead-letter recovery, and live payment/report verification remain pending. Phase 1 remains in progress.
...
- SQL Server migrations `20260820010000_multi_branch_ecommerce_fulfillment`, `20260820020000_ecommerce_network_allocation`, `20260821010000_ecommerce_hero_slides`, `20260825010000_ecommerce_store_payment_reconciliation`, and `20260825020000_ecommerce_terminal_reservation_reconciliation` are in the configured `prisma/migrations-sqlserver` deployment path and apply idempotently.
...
- Commit `57fd4b3` (`Reconcile ecommerce payments and inventory search`) was pushed to `origin/master`. `FlashERP-HQ-57FD4B3-R1-VPS-Deploy-Resolved.zip` was assembled from that clean commit with outer SHA-256 `FDF8E08C0A91C31A4D4312D8C6938677E22EFC51952C8FF7D9BB9AA9EB59C3B2`; both archive audits, the hydrated-layout regression, no-write missing-task guard, Windows PowerShell parsing, and executable packaged-Prisma-alias smoke passed. VPS deployment and live UAT remain pending.
- The ASUS ROG Gaming Desktop Ryzen 7 UAT item had 8 units on hand at East Legon, product safety stock of 5, and a stale active reservation of 2 linked to fulfilled and delivered order `SO-EAST-LEGON-MARKET-ECOM-1787654125681-977`; the storefront therefore reported `8 - 2 - 5 = 1` instead of the post-fulfilment `8 - 0 - 5 = 3`.
- Enterprise now converts any active source reservation to `CONSUMED`, `RELEASED`, or `EXPIRED` with the matching terminal sales-order event, including rolling-version Store Desktop packets that omit optional reservation fields. Store Desktop applies the same transition during fulfilment and reconciles stale terminal rows at SQLite, SQL Server, or PostgreSQL startup.
- SQL Server migration `20260825020000_ecommerce_terminal_reservation_reconciliation` idempotently repairs active ecommerce reservations and sales-order reservation headers already linked to fulfilled, cancelled, or expired sales orders. Database readiness requires the migration before promotion.
- HQ Item Dynamic, Online Store inventory and remote lookup, and Store Desktop inventory and remote HQ lookup now show the per-location on-hand, active-reserved, safety-stock, ecommerce pickup/delivery eligibility, and resulting web-sellable quantity.
- `npm run acceptance:inventory-search`, `npm run acceptance:ecommerce-multi-branch`, `npm run acceptance:desktop-sync-recovery`, Enterprise Web, Store Desktop, and Sync Core TypeScript validation, Prisma schema validation, and `npm run build:deploy` passed for the correction. The local database-readiness gate remains intentionally red until the two pending SQL Server migrations are deployed.
- Commit `231fc5a` (`Explain ecommerce stock availability`) was pushed to `origin/master`. `FlashERP-HQ-231FC5A-R1-VPS-Deploy-Resolved.zip` was assembled from that clean commit with outer SHA-256 `94581A3EA5D429441D650830A1A96CF73438628560DECB285E8EB35B9B1EB224`; both archive audits, the hydrated-layout regression, no-write missing-task guard, Windows PowerShell parsing, and packaged runtime-alias checks passed. VPS deployment, matching Store Desktop release, authenticated item-view UAT, and live verification that the ASUS item reports 3 web-sellable units remain pending.

External correction deployment evidence (2026-08-25):

- Seven external checks over a 60-second stability window returned HTTP 200 for live health, database readiness, the `accra-shop` catalogue, and storefront; database readiness remained `ready: true` throughout.
- The live readiness contract contains `20260825010000_ecommerce_store_payment_reconciliation` and `20260825020000_ecommerce_terminal_reservation_reconciliation` with no missing migrations.
- The live catalogue record for `VPS-COM-DES-085` (`ASUS ROG Gaming Desktop Ryzen 7`) reports `availableQuantity: 3`, confirming that the stale fulfilled-order reservation no longer reduces public availability.
- Authenticated HQ/Online Store item-view verification, VPS-local `FlashRMSHQ` task-action confirmation, matching Store Desktop rollout, reservation lifecycle UAT, dead-letter recovery, and payment/report reconciliation remain pending.
