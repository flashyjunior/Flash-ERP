...
```json
{
  "FLASH_ERP_DESKTOP_UPDATE_URL": "https://flashcodesolutions.com.gh/rms-update/erp/"
}
```
...
```text
Flash_ERP_Store_Desktop-<version>-x64.exe
latest.yml
*.blockmap
```
...
```text
Flash_ERP_Store_Desktop-0.1.2-x64.exe
Flash_ERP_Store_Desktop-0.1.2-x64.exe.blockmap
latest.yml
```
...
```text
https://flashcodesolutions.com.gh/rms-update/erp/latest.yml
https://flashcodesolutions.com.gh/rms-update/erp/Flash_ERP_Store_Desktop-0.1.2-x64.exe
https://flashcodesolutions.com.gh/rms-update/erp/Flash_ERP_Store_Desktop-0.1.2-x64.exe.blockmap
```

The older installer files can remain on the feed, but `latest.yml` must always point to the newest production version.
Existing desktop installations remain compatible with the underscore filename because the updater reads the installer path from `latest.yml`; the application ID and installed product name do not change.
